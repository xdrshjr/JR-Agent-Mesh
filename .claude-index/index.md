...
> Auto-generated by project-indexer | Last updated: 2026-02-13
...
- **chat-store.ts:** Conversations, messages, streaming state, model selection, dispatch mode, file attachments
  - Key actions: `startStreaming()`, `appendStreamDelta()`, `appendThinkingDelta()`, `addToolCallStart()`, `updateToolCallEnd()`, `completeStreaming()`
...
- **use-self-agent.ts:**
  - `useSelfAgent()` - Returns `{ provider, model, dispatchMode, isLoading, sendMessage, switchModel, toggleDispatch, abort, newConversation, loadConversation }`
...
| chat-handlers.ts | Handlers for chat.send, chat.abort, chat.steer, chat.switch_model, etc. |
...
| sidebar.tsx | Left nav sidebar with running agents list |
| nav-menu.tsx | Navigation links to Chat, Agents, Settings |
| top-bar.tsx | Header bar with logo and sound toggle |
...
| input-area.tsx | Chat input with file upload (drag-drop, limits enforced) |
